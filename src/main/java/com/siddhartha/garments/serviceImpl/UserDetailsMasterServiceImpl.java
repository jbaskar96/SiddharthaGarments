...
public class UserDetailsMasterServiceImpl implements UserDetailsMasterService{
	...
	public CommonResponse saveUserDetails(UserDetailsRequest req) {
		...
		try {
			...
			String role ="";
			if("1".equalsIgnoreCase(req.getUserType())) {
				role="ADMIN_0";	
			}else if("2".equalsIgnoreCase(req.getUserType())) {
				role ="USER_0";
			}else if("3".equalsIgnoreCase(req.getUserType())) {
				role ="OPT_0";
			}
			...
			if(error.isEmpty()) {
				...
				UserDetailsMaster detailsMaster = UserDetailsMaster.builder()
						.userId(StringUtils.isBlank(req.getUserId())?role+userId:req.getUserId())
						...
						.loginId(StringUtils.isBlank(req.getUserName())?"": req.getUserName())
						.build();
				UserDetailsMaster savedData =repository.save(detailsMaster);
				
				
				if(!"3".equalsIgnoreCase(req.getUserType())) {
					...
					loginMaster.setUsertype(req.getUserType());
		...
		try {
			...
			if(!list.isEmpty()) {
				...
				list.forEach(p ->{
					...
					map.put("Status", p.getStatus().equals("Y")?"ACTIVE":"DEACTIVE");
					res.add(map);
	...
	public CommonResponse editUseDetails(String userId) {
		...
		try {
			...
			if(!userId.startsWith("OPT")) {
				...
				List<Tuple> userList =criteriaQueryImpl.getOperatorList(userId);
				if(!userList.isEmpty()) {
					Tuple tuple =userList.get(0);
					HashMap<String, String> map = new HashMap<String, String>();
					map.put("UserId", tuple.get("userId")==null?"":tuple.get("userId").toString());
